...
class Laporan_penduduk_model extends MY_Model
{
    ...
    protected function hitung_persentase(&$data, $semua)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['persen']  = persen($data[$i]['jumlah'] / 1);
            $data[$i]['persen1'] = persen($data[$i]['laki'] / 1);
            $data[$i]['persen2'] = persen($data[$i]['perempuan'] / 1);
        }
    ...
    protected function persentase_semua($semua)
    {
        ...
        $semua['persen']  = persen(($semua['laki'] + $semua['perempuan']) / 1);
        $semua['persen1'] = persen($semua['laki'] / 1);
        $semua['persen2'] = persen($semua['perempuan'] / 1);
